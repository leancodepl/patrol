...
1. Install `patrol_cli` executable:
  ...
  <Info>
     Make sure to add `patrol_cli` to your `PATH` environment variable.
  </Info>
...
2. Verify that installation was successful and your enviroment is set up properly:
   ...
   <Warning>
        Make sure that the `patrol doctor` does not show any errors for the
...
<Accordion defaultOpen title="Android setup">
      ...
      <Info>
        `patrol` package requires Android SDK version 21 or higher.
      </Info>
      ...
      <Info>
        Remember to replace `/com/example/myapp/` with the path created by your app's 
        package name.
      </Info>
...
up. To run `integration_test/example_test.dart` on a connected Android, iOS or macOS device:
      ...
      If something went wrong, please proceed to the [FAQ](#setup-faq) section which might
...
<Accordion title="Testing fails with error inside `integration_test/test_bundle.dart`">
  The reason is a mismatch of `patrol` and `patrol_cli` versions. Make sure that you 
...
within the patrol test.">
  ...
  repeat the same code in every test.
</Accordion>
...
<Accordion title="Running patrol test fails containing `Unsupported class file major version` error">
  ...
  Change your $JAVA_HOME path to use JetBrainsRuntime.
</Accordion>
...
<Accordion title="When I run tests the emulator is getting cloned">
...
[Patrol Discord Channel](https://discord.gg/ukBK5t4EZg).
