...
<Accordion defaultOpen title="iOS setup">
      ...
      8. Open your Xcode project and make sure that `Runner` and `RunnerUITests` 
         target have correct base Build Configurations:
      ...
      11. Paste this code into the first `xcode_backend build` Build Phase:

      ```
      /bin/sh "$FLUTTER_ROOT/packages/flutter_tools/bin/xcode_backend.sh" build
      ```

      12. Paste this code into the second `xcode_backend embed_and_thin` Build Phase:
