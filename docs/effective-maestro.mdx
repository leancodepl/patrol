...
find the widget you want using classes.
...
In some cases using classes for finding widgets might be OK, but let's see
another example:
...
This works, but if some changes are made to the `Select` widget, your test will
break. For example, if its type parameter is changed from `String` to some
specialized `PersonData` model. This change would case the above line to break,
and you'd have to update it to:
...
> Have tester's midset.

Treat your finders as if they were the tester's eyes.

### CONSIDER having a global file where all keys are defined

The number of keys will get bigger as your app grows and you write more tests.
To keep track of them, it's a good idea to keep all keys in some global
`keys.dart` file.
