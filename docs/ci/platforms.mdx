...
An example of a more advanced use case that is not possible to accomplish on
device labs is scanning a QR code. One of the apps we worked on had this
feature, and we wanted to test it because it was critical part of the flow. When
you have access to the shell filesystem (which you do have in the "manual"
approach, and don't have in the "device lab" approach), you can easily replace
the scene that is visible in the camera's viewfinder:
...
When running on Firebase Test Lab, your app is automatically granted all
permissions. If you want to test the flow of requesting permissions, you're out
of luck.
...
Android emulator on GitHub Actions. Let us tell you this: Running an Android
emulator on default GitHub Actions runner is a bad idea. It is slow to start and
unstable. Apps crash randomly. And it's slow. Really, really slow. Do not do it
unless you crave for pain. We tried to mitigate its instability by using Test
Butler, but it comes with its own restrictions, most notably, it doesn't allow
...
There are many more CI/CD platforms. Some of the more prominent alternatives
include [CircleCI], [CirrusCI], and [GitLab CI/CD]. We did not check whether
running Flutter integration tests works fine on them, so we can't say more about
them. If you did, we (and other Patrol users) will be happy to hear about your
experiences!
