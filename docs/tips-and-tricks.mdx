...
You can also use the one-liners below, but keep in mind that they build and test
the app 10 times, while `patrol drive --repeat 10` builds every test target only
once and run it 10 times.
...
- [Patrol issue #628][628]

## Bundling many tests into a single app binary

A new application binary (`.apk` for Android and `.app` for iOS) is built for
every Dart test file in the `integration_test` directory. This is [slow and
suboptimal][114541_comment], but there's a workaround that makes it possible to
bundle many tests into a single application binary.

Let's say that you have 2 tests: `add_comment_test.dart` and
`follow_user_test.dart`. To bundle these tests:
...
Bear in mind that this can increase test flakiness since many test run in the
single app. Normally, the app in reinstalled and its data is cleared between
tests, which prevents many issues.

See also:

- Bundled test in Patrol's example
  app[https://github.com/leancodepl/patrol/blob/master/packages/patrol/example/integration_test/features/services/all_services.dart]
- [Flutter issue #115751](https://github.com/flutter/flutter/issues/115751)

[#628]: https://github.com/leancodepl/patrol/issues/628
...
[114541_comment]: https://github.com/flutter/flutter/issues/114541#issuecomment-1301094425

https://github.com/leancodepl/patrol/blob/master/packages/patrol/example/integration_test/features/services/airplane_mode_test.dart
