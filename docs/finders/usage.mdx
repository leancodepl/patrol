...
a short while until the data is loaded.

Sometimes though, you want to assert that a certain widget is visible, but take
no further action. To accomplish this, use Patrol's [waitUntilVisible()] method
(it's used by `tap()` and `enterText()` under the hood):
