...
### `scrollUntilVisible()` example

The `//` text in the example code, makes concrete an alternative possible syntax
 for those who benefit from concrete examples.
```
final targetText = 'text-to-search-for';
await $
    .scrollUntilVisible(
      finder: find.textContaining(targetText, skipOffstage: true),
      // view: $(const Key('someKey')).$(
      view: $(#someKey).$(
        Scrollable,
      ),
      maxScrolls: 10,
