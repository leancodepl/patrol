...
but you can change it to `pump` or `trySettle`. While `settle` and `pump` simply
refer to Flutter's built-in methods, `trySettle` is available only in Patrol.
How is it different from other ones?
...
This default is reasonable and what you want most of the time. Unfortunately, it
can sometimes cause problems in more complicated UIs, where there are more than
a single `Scrollable` widget visible at the same time. In such cases we strongly
recommend to explicitly specify the `Scrollable` that `scrollTo()` should
scroll, to avoid the problem of the target widget never becoming visible because
the wrong scrollable was scrolled.
