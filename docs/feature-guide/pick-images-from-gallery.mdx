...
Patrol provides functionality to pick an image from the Android and iOS gallery using the `pickImageFromGallery` or pick multiple
images with `pickMultipleImagesFromGallery` method from the native automator.
...
1. Opens the gallery.
2. Selects multiple images (user needs to specify indexes of images to select).
3. Confirms the selection.

You can provide a custom selector for the image using the `imageSelector` parameter. If no custom selector is provided, default selectors will be used.
