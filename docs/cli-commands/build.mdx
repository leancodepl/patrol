...
The `--full-isolation` flag enables full isolation between test runs:
- **iOS**: Uninstalls the app before running each test (except for the first one)

<Info>
    It is effectively used only when building the iOS app.
    The behavior on Android can be configured through the `patrol test` command or within the `build.gradle` file.
