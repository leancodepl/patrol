...
<Info>Dart file must end with `_test.dart` to be considered a test</Info>

To build the app for testing on Android, you have to run `patrol build android
--target <your file>` 4 times, to get 4 different app binaries, that have to be
separately uploaded to a device farm. This is needlessly inefficient for apps
with many tests, but it's a design flaw in the way integration testing in
Flutter works.
...
large apps with many integration tests.
