...
- **BrowserStack App Automate**: enable the Espresso `debugscreenshots: true`
  capability; images appear in the Screenshots tab.
...
<Info>
    ...
    APK as well as `patrol test`. `url_safe_test_names: true` sanitizes the
    reported JUnit names so screenshot paths render on BrowserStack (a stopgap;
    off by default, reported names unchanged when off).
</Info>
