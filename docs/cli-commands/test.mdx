...
- every test with its status and duration, expandable to reveal its Patrol steps
  (each with its own duration) and the logs printed from each step,
- for a failure, the exception folded into the step that broke: the row shows the
  first line, and clicking it unfolds the full message and the stack trace,
...
<Info>
    ...
    same on Android, iOS and web. `patrol test-without-building` accepts the same
    two flags.
</Info>
