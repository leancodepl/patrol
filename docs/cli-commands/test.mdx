...
One `.mp4` per test case is saved to `<test-directory>/videos` (override with
`--video-output-dir`), named `patrol_<test>_<device>_<timestamp>.mp4` on every
platform.

- **Android**: emulators, and physical devices depending on the vendor (some
  block `screenrecord`). Recorded with `adb shell screenrecord` while the test
  runs. `--video-size` and `--video-bit-rate` apply here only.
- **iOS**: simulators and physical devices. XCTest records the screen of every
  test on its own; `patrol test` extracts those recordings from the `.xcresult`
  bundle after the run, so they show up when the run finishes, not while it's
  going. Requires Xcode 16 or newer (`xcrun xcresulttool export attachments`).
  `patrol develop` records with `xcrun simctl` instead, on simulators only.

On a device farm like Firebase Test Lab, `--record-video` doesn't apply (the
tests run via `gcloud`, not `patrol test`), but the iOS `.xcresult` still holds
the per-test recordings of failing tests. Pull it from the native artifacts the
same way as [iOS screenshots](#ios) — the exported attachments include the
`.mp4` recordings.
...
failure (opt-in) and on demand from a test. On Android this requires `patrol_cli`
4.8.0 and `patrol` 4.10.0 or newer:
...
`await $.takeNativeScreenshot('tag')`. With `screenshot_on_failure`, the screen is
captured at the failure moment (via the Dart failure hook, before teardown);
`takeNativeScreenshot('tag')` captures whenever the call executes, so you can grab
before/after states on purpose. Either way the image is captured natively and
attached to the test's `.xcresult` bundle, which `xcodebuild` writes on the host —
so it works on simulators and physical devices alike, with no on-device pull.
