...
(`patrol.emit_test_manifest: true`), you can re-run already-built tests **without
rebuilding** with `--no-build`, reusing the artifacts from a previous
`patrol build`:
...
`--no-build` requires `emit_test_manifest` and a prior `patrol build`. It runs the
tests via `xcodebuild test-without-building` on iOS and `adb shell am instrument`
on Android, skipping compilation entirely — handy for a fast re-run of a single
failing test. `--only` takes the exact Dart test name and can be repeated; omit it
to run everything.
