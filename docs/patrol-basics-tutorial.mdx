...
Before writing any tests, make sure you [install the Patrol CLI](https://patrol.leancode.co/getting-started#install-patrol_cli). Then just clone the following repository from GitHub to follow along. The app we’re going to be testing is fully functional and ready to be tested, with Patrol already configured.

<Info>
To learn how to set up Patrol for your own project, check out the [Patrol Setup Docs](https://patrol.leancode.co/getting-started).
</Info>

<Info>
Clone the [STARTER PROJECT](https://github.com/ResoCoder/patrol-basics-tutorial) to follow along.
</Info>
...
<Info>
Native automation allows you to interact with the OS your Flutter app is running on. Patrol currently supports Android, iOS and macOS native interactions. [Learn more from the docs](https://patrol.leancode.co/native/overview)
</Info>
