...
To achieve full isolation between test runs, you can use different approaches depending on your platform:

#### Android

Enable the `clearPackageOption` in your Android configuration:
...
For iOS testing, use the `--clear-package-data` CLI flag to uninstall the app before running each test:

```bash
patrol test --clear-package-data
```

This flag uninstall the app before each test run (except of the first one), providing similar isolation benefits to
