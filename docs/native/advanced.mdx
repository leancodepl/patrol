...
To achieve full isolation between test runs, you can use the `--full-isolation` flag or configure it directly in
your build configuration.

#### Using the --full-isolation flag

<Warning>
    This functionality is experimental on iOS.
</Warning>

The `--full-isolation` flag is supported on both Android and iOS:
...
This flag:
- **Android**: Clears package data before running each test
- **iOS**: Uninstalls the app before running each test (except for the first one)
...
When both gradle configuration and `--full-isolation` flag are used:
- If `clearPackageData` is **not set** in gradle (default `false`): `--full-isolation` sets it to `true`
- If `clearPackageData` is **set to `true`** in gradle: `--full-isolation` has no effect (already `true`)  
- If `clearPackageData` is **set to `false`** in gradle: `--full-isolation` overrides it to `true`
