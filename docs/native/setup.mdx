...
<Info>
  If your app has different name on iOS and Android, then specify the names like
  so:
...
<Tabs 
  ...
  <TabItem value="ios">
      ...
      <Info>
        In addition to `--flavor`, `patrol test` optionally accepts a bunch of
        iOS-specific arguments, such as `--scheme`, `xcconfig`, and
        `--configuration`, in case your app is set up in a less standard way.
      </Info>
