...
It's important to note that Flutter's default functions, such as [findsNothing]
and [findsOneWidget], check if the widget is present in the widget tree, not if
it is visible to the user, which is usually what we're interested in.
...
can specify globally in [PatrolTestConfig] or as argument to the `tap()` method)
and taps on the first widget as soon as it becomes visible. This lets you get
rid of fixed timeouts and test your app just like a real user would.
