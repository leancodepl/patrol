...
The `scrollTo()` method is simple to use, yet very poweful. It scrolls to the
really handy when you _know_ that some widget will become visible if you

Here's how it works:

1. Waits for at least 1 Scrollable widget to become visible
2. Scrolls that Scrollable widget in its scrolling direction until the target
   widget becomes visible

`scrollTo()` is really powerful, but there's 1 important thing to keep in mind
that can cause you problems. This thing is:
...
than a single `Scrollable` widget is present in the widget tree. In such cases
we strongly recommend explicitly specifying the `Scrollable` that `scrollTo()`
should scroll, to avoid the problem of the target widget never becoming visible
because the wrong scrollable was scrolled.

Let's consider this very simple app:
...
widget with the `index: 100` text (that is the last `Text` widget built by the
...
The above line will scroll the second `Scrollable` and find the widget with
`"index: 100"` text.

You might be wondering why we `scrollTo(scrollable: $(#listView2))` is not
enough? Why is it needed to look for a `Scrollable` widget inside the widget
with the `listView2` key?
...
problem][listview_problem].

[listview_problem](https://github.com/flutter/flutter/issues/88762#issuecomment-1046073880)
