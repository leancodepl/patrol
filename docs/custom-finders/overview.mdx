...
you have to do is to add package `maestro_test` to `dev_dependencies` in your
project's `pubspec.yaml`.

```bash
flutter pub add maestro --dev
```

### Writing

When using Maestro's custom finders, you use `maestroTest` function instead of
`testWidgets`.

The biggest difference between `testWidgets` and `maestroTest` is the type of
the _tester_ you get in your callback. `maestroTest` gives you `MaestroTester`,
...
```dart
maestroTest('sign in', (MaestroTester $) async {
  await $(#signUpButton).tap();
}
```
...
You run widget tests using Maestro's custom finders the same way as you run
normal widget tests.
