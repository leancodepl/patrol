...
<Info>
Add these to `.gitignore` — they should not be committed:
```
**/test_bundle.dart
.patrol.env
```
`test_bundle.dart` is generated by Patrol on every run (in the test directory for mobile runs, in the project root for web). `.patrol.env` is where you keep your Patrol dart-defines, which often include secrets.
</Info>
