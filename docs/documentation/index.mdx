...
<Warning>
  ...
  ```
    ...
    <Accordion title="iOS setup continuation for physical devices">
      ...
      Going further, we need to sign app and tests. Let's assume that your team joined Apple Developer Program and have already an App ID, profile and certificate for your app. We need to do the same for `RunnerUITest`.
      <Warning>
      ...
      <Steps>
        <Step>
          In [Apple Developer Portal](https://developer.apple.com) create new Identifier (App ID) with your app's bundle ID and with `.RunnerUITests.xctrunner` ending. It should look like e.g.: `your.organization.app.RunnerUITests.xctrunner` (Remember to swap `your.organization.app` with your app's one).
        </Step>
        <Step>
