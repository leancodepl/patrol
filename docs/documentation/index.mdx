...
<Steps>
    ...
    <Info>
        **Native failure screenshots (Android/BrowserStack)**: Set
        ...
        the device (to `Download/screenshots/<class>/<method>/`); a device farm
        collects it. On BrowserStack App Automate you must enable the Espresso
        `debugscreenshots: true` capability, which collects those images into the
        Screenshots tab. It is off by default and currently applies to Android
        only (a no-op on iOS). It works in the `patrol build android` APK as well
        as `patrol test`/`develop`. You can also capture on demand from a test
        with `await $.takeScreenshot('tag')`.
        ...
        execution. It is a stopgap so screenshot URLs render on BrowserStack
        (which currently can't render paths with spaces/commas). Off by default;
        when off, reported test names are unchanged.
