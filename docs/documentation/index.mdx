...
<Warning>
  ...
  ```
    ...
    <Accordion title="iOS setup continuation for physical devices">
      <Steps>
        <Step>
          In Apple Developer Portal create new App ID with `.xctrunner` ending. It should look like e.g.: `your.organization.app.RunnerUITests.xctrunner` (Remember to swap `your.organization.app` with your app's one).
        </Step>
        <Step>
          In Apple Developer Portal generate new profile with type Development that signs `your.organization.app.RunnerUITests.xctrunner`.
        </Step>
        <Step>
          In Apple Developer Portal generate new certificate that covers newly created profile.
        </Step>
        <Step>
        ...
        <Step>
          Import newly generated certificate and profiles for both Targets (Runner and RunnerUITests).
        </Step>
        <Step>
          Try it out by running `patrol build ios --release`. If tests have been built successfully you're ready for action!
        </Step>
      </Steps>
    </Accordion>
