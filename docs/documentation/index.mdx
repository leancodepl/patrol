    ...
    <Info>
        **Native failure screenshots (device farms)**: Set
        ...
        - **BrowserStack App Automate**: enable the Espresso
          ...
          tab. Also enable build-time test discovery (`emit_test_manifest`)
          so each test reports a URL-safe name — BrowserStack currently can't
          render screenshot URLs with spaces/commas, which the default
          `runDartTest[...]` names contain.
