  ...
  ```
    ...
    <Accordion title="iOS setup">
      <Steps>
        ...
        <Step>
          **(Swift Package Manager only)** In Xcode, go to **RunnerUITests > Build Settings**,
            search for **Header Search Paths**, and add:

          ```
          $(SRCROOT)/Flutter/ephemeral/Packages/.packages/patrol/Sources/patrol/include
          ```

          In case you require backward compatibility with CocoaPods, add the following search path as well:
          ```
          $(inherited)
          ```

          <Warning>These search paths must be added for each configuration (Debug, Release, Profile) and scheme (e.g. dev, prod).</Warning>

          ![Xcode RunnerUITests header search paths](/assets/ios_spm_header_search_paths.png)
        </Step>
