...
<Info>
Add Patrol's generated files to `.gitignore` — they should not be committed:
```
**/test_bundle.dart
.patrol.env
```
`test_bundle.dart` is generated by Patrol on every run (in the test directory for mobile runs, in the project root for web). `.patrol.env` holds your Patrol dart-defines, which often include secrets.
</Info>
