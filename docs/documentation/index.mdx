    ...
    <Info>
        **Native failure screenshots (device farms)**: Set
        ...
        - **BrowserStack App Automate**: enable the Espresso
          `debugscreenshots: true` capability; images appear in the Screenshots tab.
        ...
        `url_safe_test_names: true` sanitizes the reported Android JUnit test
        names (`[^A-Za-z0-9._-]` → `_`) while keeping the original names for
        execution. It keeps screenshot paths free of spaces/commas — mainly a
        stopgap for BrowserStack, which currently can't render such URLs. Off by
        default; when off, reported test names are unchanged.

        ```yaml title="pubspec.yaml (optional)"
        patrol:
          app_name: My App
          screenshot_on_failure: true
          url_safe_test_names: true # only needed until BrowserStack fixes path rendering
          android:
            package_name: com.example.myapp
        ```
    </Info>
