...
hidePrevious: true
# The platform setup sections are accordions, not markdown headings, so they are
# absent from the auto-generated "On this page" TOC. Surface them (sections only —
# their individual steps would overwhelm the list) nested under the "Setup" entry.
tocExtra:
  - title: Android setup
    url: "#android-setup"
    depth: 3
    after: "#setup"
  - title: In-app Android runner
    url: "#old-android-setup"
    depth: 3
    after: "#setup"
  - title: iOS setup
    url: "#ios-setup"
...
<Info>
     **Want your AI agent to set Patrol up for you?** Install the [`patrol-setup` agent skill],
     or simply paste [this prompt] into the conversation — either way it follows the steps
     below and gets a first test passing on an Android emulator.
</Info>

<Steps>
    <Step id="install-patrol-cli">
    Install `patrol_cli`:
    ```
...
<Accordions>
<Accordion defaultOpen id="android-setup" title="Android setup (for projects using Kotlin DSL)">
  <Steps>
    <Step id="include-patrol-test-modules">
      Add the Patrol test APK and its native harness to `android/settings.gradle.kts`.
      The harness path is resolved from Flutter's existing `:patrol` plugin project,
      so it also works when Patrol is installed from the pub cache.
      ...
      For `settings.gradle` (Groovy), use:
      ...
      include ':patrol_test_harness'
      project(':patrol_test_harness').projectDir =
          new File(project(':patrol').projectDir, '../android_test')
      ```
    </Step>

    <Step id="create-patrol-test-module">
      Create `android/patrolTest/build.gradle.kts`. This is a self-instrumenting
      test-only APK. It copies the app's flavor dimensions, product flavors, build
      types, and SDK levels.

      ```kotlin title="android/patrolTest/build.gradle.kts"
      ...
      val appAndroid =
          project(":app").extensions.getByType(ApplicationExtension::class.java)
      ...
      android {
          ...
          // Resolve custom build types against a harness variant.
      ...
      `patrol_test.gradle` assigns each test APK a package ID and embeds the
      matching app variant's `applicationId` and clear-data setting as manifest
      metadata.

      The orchestrator gives every Dart test a fresh instrumentation process and a
      freshly launched app. With `clearPackageData` set as above, Patrol wipes the
      **app's** data before each test (including runtime permissions).
      `:patrol_test_harness` contributes the test APK's required `INTERNET`
      and `QUERY_ALL_PACKAGES` permissions.
    </Step>

    <Step id="create-mainactivitytest">
      Create
      `android/patrolTest/src/main/java/com/example/myapp/patrol_test/MainActivityTest.java`.
      Replace the package declaration and directory with
      `<your applicationId>.patrol_test`.

      ```java title="MainActivityTest.java"
      package com.example.myapp.patrol_test;
...
<Accordion id="old-android-setup" title="In-app Android runner">
  This layout runs instrumentation from `:app`'s `androidTest` source set.
  <Steps>
    <Step id="create-androidtest-dir">
    ...
    <Step id="configure-in-app-runner">
      Keep the runner and Orchestrator configuration in `android/app/build.gradle.kts`
      (or `build.gradle`):
...
<Warning>
  ...
  ```
    ...
    <Accordions>
    <Accordion id="ios-setup" title="iOS setup">
      <Steps>
        <Step id="open-xcworkspace">
          Open `ios/Runner.xcworkspace` in Xcode.
        </Step>
          
        <Step id="create-test-target">
        ...
        <Step id="configure-runner-uitests">
        ...
        <Step id="set-xcode-backend-build">
        ...
        <Step id="set-xcode-backend-embed">
        ...
        <Step id="disable-parallel-execution">
          ...
          <Info>
            The video shows a scheme that does not use test plans, which is the
            default for schemes generated by Flutter. If the **Test** tab of your
            scheme editor lists test plans instead of test targets (for example,
            after you converted the scheme with **Convert to use Test Plans...**
            or set up BrowserStack), disable parallel execution in the test plan
            instead: open the `.xctestplan` file, go to the **Tests** tab, click
            **Options** next to `RunnerUITests` and uncheck
            **Execute in parallel (if possible)**. In the JSON file this removes
            `"parallelizable" : true` from the `RunnerUITests` entry in `testTargets`.
          </Info>
        </Step>

        <Step id="disable-script-sandboxing">
    ...
    <Accordion id="macos-setup" title="macOS setup">
    ...
    <Warning>
      ...
      <Steps>
        ...
        <Step id="enable-app-sandbox-connections">
    ...
    <Step id="create-integration-test">
...
<Info>
`test_bundle.dart` is auto-generated by Patrol and should not be committed. Add it and your `.patrol.env` secrets to `.gitignore`:
```
**/test_bundle.dart
.patrol.env
```
...
<Accordion id="faq-app-not-running-in-test" title="'example_test.dart' passed but I can't get my application to run within the patrol test.">
...
<Accordions>
<Accordion id="faq-android-package-name" title="Where do I get 'package_name' from?">
  Go to `android/app/build.gradle` and look for `applicationId` in `defaultConfig` section.
</Accordion>

<Accordion id="faq-unsupported-class-file-version" title="Running patrol test fails containing 'Unsupported class file major version' error">
...
<Accordion id="faq-build-stuck" title="Build gets stuck during 'patrol test' or 'patrol build android'">
...
<Accordions>
<Accordion id="faq-ios-bundle-id" title="Where do I get `bundle_id` from?">
...
<Accordion id="faq-simulator-cloned" title="When I run tests the simulator is getting cloned">
  Make sure that you disabled "Paralell execution" for **all schemes** in Xcode.
  See [this video] for details.
</Accordion>

<Accordion id="faq-test-stops-wait-for-idle" title="Running test stops on 'Wait for com.example.myapp to idle'">
  Search for a `FLUTTER_TARGET` in your project files and remove it (both value and key)
  from *.xcconfig and *.pbxproj files.
</Accordion>

<Accordion id="faq-real-app-opened-instead" title="When running a test, real app without test is opened instead">
  Search for a `FLUTTER_TARGET` in your project files and remove it (both value and key)
  from *.xcconfig and *.pbxproj files.
</Accordion>

<Accordion id="faq-ios-deployment-version-mismatch" title="Build is failing with errors on mismatching iOS deployment versions">
...
<Accordion id="faq-generate-flutter-target" title="How to generate FLUTTER_TARGET after removing it from *.xcconfig">
...
Using AI? [Patrol MCP] lets an agent run your tests, and [agent skills] teach it to write them.

[native automation]: /documentation/native/usage
[Patrol MCP]: /documentation/other/patrol-mcp
[agent skills]: /documentation/other/agent-skills
[this prompt]: https://raw.githubusercontent.com/leancodepl/patrol/refs/heads/master/skills/patrol-setup/SKILL.md
[`patrol-setup` agent skill]: https://github.com/leancodepl/patrol/tree/master/skills
