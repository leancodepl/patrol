...
<Steps>
    ...
    <Info>
        **Native failure screenshots (Android/BrowserStack)**: Set
        ...
        of the failing screen when a test fails. This is intended for
        BrowserStack App Automate, where Espresso `debugscreenshots` collects
        images from `Downloads/screenshots/<class>/<method>/`. It is off by
        default and currently applies to Android only (a no-op on iOS). It works
        in the `patrol build android` APK as well as `patrol test`/`develop`.
        You can also capture on demand from a test with `await $.takeScreenshot('tag')`.
