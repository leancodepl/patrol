...
- On Android: set `clearPackageData` to `true` in `android/patrolTest/build.gradle.kts`,
...
Set `clearPackageData` on the Patrol test module to clear the app under test
before each test:
...
<Tabs
  defaultIndex={0}
  items={['Android Studio', 'Xcode', 'Firebase Test Lab', 'Gradle', 'fastlane scan']}
>
  ...
  <Tab value="Gradle">
    ...
    You can find it in
    `build/patrolTest/reports/androidTests/connected/index.html`
    (`:patrolTest` layout) or
    `build/app/reports/androidTests/connected/index.html`
    (in-app `androidTest` layout).
