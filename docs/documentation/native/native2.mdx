...
platforms proved problematic because **iOS and Android use different selector arguments** (Android's `resourceName` vs iOS's `label`) and a single selector approach couldn't effectively handle the fundamental differences
