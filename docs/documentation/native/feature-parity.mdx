...
<Warning>
  macOS support is still in alpha. A small set of platform automation APIs is
  available via `$.platform.macos` (native tap/wait, `NSAlert` dialogs, and
...
These features are available via `$.platform.macos`:

| **Feature**                  | **macOS**   |
| ---------------------------- | ----------- |
| Tap                          | ✅          |
| Wait until visible           | ✅          |
| Handle `NSAlert` dialogs     | ✅          |
| Tap application menu items   | ✅          |
