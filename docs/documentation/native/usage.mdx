...
skips these nodes as well. To interact with one, tap its coordinates with `tapAt`. Note that
`tapAt` takes fractions of the **display** size (`uiDevice.displayWidth/Height`), not of the
root node's bounds, which usually exclude the system bars.
