...
<Steps>
  ...
  <Step>
    ...
    <Warning>
      When running tests on a physical iOS device, a failed test can make
      `xcodebuild` show a `password:` prompt while collecting diagnostics. The
      command will stay blocked until the user interrupts it (e.g. by pressing `Ctrl+C`).

      To avoid this, set `diagnosticCollectionPolicy` to `Never` in
      `defaultOptions` in your `ios/TestPlan.xctestplan` file:
