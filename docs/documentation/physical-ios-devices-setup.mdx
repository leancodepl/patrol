...
<Steps>
  ...
  <Step>
    ...
    <Info>
      Patrol CLI disables Xcode's post-test diagnostic collection because it can block a completed simulator suite for 10 minutes or hang at a `password:` prompt on a physical device.

      When running tests directly with `xcodebuild` or using an older Patrol CLI, add `"diagnosticCollectionPolicy": "Never"` to the `defaultOptions` section in your `.xctestplan` file (e.g., `ios/TestPlan.xctestplan`):
