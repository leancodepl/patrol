...
<Info>
  **Requires `patrol_cli` 4.8.0 and `patrol` 4.10.0 or newer.** Both sides have to
  ...
  [compatibility check](/documentation/compatibility-table). The feature first
  shipped in `patrol_cli` 4.7.0 / `patrol` 4.9.0 with a different iOS runner macro
  and different generated test names — see [the upgrade note](#setup) if you're
...
`shard: concurrency`) or [BrowserStack's XCUITest sharding](/documentation/integrations/browserstack#sharding).
