...
2. This step is only required if your tests are not located in the `patrol_test` directory and/or do not use the `_test.dart` suffix. 

In your project's pubspec.yaml, add two lines to patrol section:
```yaml title=pubspec.yaml
patrol:
  flavor: dev
  test_directory: patrol_test # add this line
  test_file_suffix: _test.dart # add this line
  android:
...
3. If you have a method wrapping `patrolTest()`, you need to add an annotation to it:
