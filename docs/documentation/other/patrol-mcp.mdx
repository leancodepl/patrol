...
`patrol_mcp` uses `patrol_cli` as a library, resolved in your own
`pubspec.yaml` alongside `patrol`, and both of them pull `patrol_log` — so the
`patrol_cli` row reaches your `patrol` version as well. `patrol_mcp` 0.2.1
needs `patrol` 4.7.0 to resolve, and `patrol_cli` checks the app's `patrol`
before every run, where 4.8.0 accepts 4.9.0 or newer. See the
