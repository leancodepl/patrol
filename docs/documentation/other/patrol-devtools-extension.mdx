...
A powerful Flutter DevTools extension that allows you to **inspect native UI elements** on Android and iOS devices, and the **page's DOM** on web, while developing your Patrol tests. This extension provides a tree view of the UI hierarchy your selectors resolve against, making it easier to write accurate selectors for your integration tests.
...
- **Cross-platform Support**: Works with Android, iOS and web applications
...
On web there is no native view hierarchy — the inspector shows the page's DOM,
which is exactly what web selectors resolve against. With semantics enabled
(Patrol's default), Flutter renders `<flt-semantics>` elements carrying the roles
and labels that finders match, so this is the first place to look when a selector
doesn't find what you expect.

| Property | Description | Example |
|----------|-------------|---------|
| `testId` | `data-testid` attribute (most reliable) | `login-btn` |
| `id` | DOM `id` attribute | `flt-semantic-node-35` |
| `role` | ARIA role | `button` |
| `ariaLabel` | Accessible label | `"Enter text"` |
| `text` | Text belonging directly to the element | `"Go to Page 1"` |
| `tagName` | DOM tag name | `FLT-SEMANTICS` |

These map onto `WebSelector`, whose full list of properties can be found:
https://pub.dev/documentation/patrol/latest/patrol/WebSelector-class.html
