...
- Fix `pickImageFromGallery` on Android API 36: when the photo picker keeps the picker open after selecting a single image, tap "Done" to confirm. The tap is best-effort, so devices/emulators whose picker auto-confirms (no "Done" button) keep working. (#3254)
