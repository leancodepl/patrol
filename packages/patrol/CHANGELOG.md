...
- Extend native screenshots to iOS. The runner attaches a `patrol_failure` screenshot to every failing test's `.xcresult`, and `$.takeNativeScreenshot('tag')` is now supported on iOS too (attached as `patrol_<tag>`). Screenshots ride inside the result bundle written on the host, so this works on simulators and physical devices (e.g. Firebase Test Lab); set `screenshot_on_failure: true` to have `patrol test` extract them to `<test-directory>/screenshots`. (#3271)
