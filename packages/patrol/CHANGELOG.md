...
- Android: reject a test name containing `/` with a message naming the offending test, instead of letting it crash Android Test Orchestrator. The orchestrator writes one output file per test case and refuses a file name with a path separator, which killed the run before any test executed - reported as `0 tests` with no reason given. The message is sent on the `patrol_log` channel, so `patrol test` prints it without `--verbose`. Build-time discovery (`patrol.emit_test_manifest`) is unaffected: it hands the orchestrator sanitized method names, so such test names keep working there. (#1839)
