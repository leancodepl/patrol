...
- **Breaking for build-time test discovery on iOS:** the generated `.inc` now declares one
  `XCTestCase` subclass per Dart test file. In `RunnerUITests.m`, replace the
...
- Add `PatrolJUnitRunner.setUpGenerated` for the Android classes generated by build-time discovery:
  it never stands down for the host class and launches the app once per instrumentation process.
- Add `patrolTargetPlatform`, which reports the platform a test run targets. Use it instead of
  ...
  registered (most often `skip:`), so build-time discovery on the host matches the device. (#3241)
- Reply to the native runner when it asks for a test this app skips or doesn't have (reported as
  skipped, or as an assumption failure on Android) instead of leaving it waiting forever. (#3241)
- Android: the runtime-discovery host class stands down when a generated class is compiled into the
  same APK, so tools that instrument the APK directly (Firebase Test Lab, saucectl, emulator.wtf,
  Marathon) no longer run the whole suite twice.
