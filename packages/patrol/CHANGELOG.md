...
- Reject a test name containing `/`, with a message naming the offending test, instead of letting it crash Android Test Orchestrator. The name is rejected on every platform. Build-time test discovery (`patrol.emit_test_manifest`) is exempt on purpose: it compiles sanitized native method names, so such a test name has nothing to break there. (#1839)
