...
- **Breaking for build-time test discovery on iOS:** the static runner is now a base class holding
  the shared infrastructure, and the generated `.inc` declares one `XCTestCase` subclass per Dart
  test file. Replace the `PATROL_INTEGRATION_TEST_IOS_RUNNER_STATIC_BEGIN`/`_END` pair in
  `RunnerUITests.m` with `PATROL_INTEGRATION_TEST_IOS_RUNNER_STATIC_BASE(RunnerUITests)` followed by
  the `#include`. The test server now starts once per process rather than once per test class.
- Add `PatrolJUnitRunner.setUpGenerated`, which the Android classes generated by build-time
  discovery call instead of `setUp` plus `waitForPatrolAppService`. It never stands down for the host
  test class, so generated tests can no longer be disabled by the very mechanism that disables the
  host. It launches the app and the test server once per instrumentation process; the remaining
  generated classes reuse them, so running several classes without the orchestrator works.
