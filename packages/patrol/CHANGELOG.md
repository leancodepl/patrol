...
- Add BrowserStack-friendly Dart coverage collection. With `--dart-define=PATROL_BS_COVERAGE=true` (optional `PATROL_BS_COVERAGE_PACKAGES=<regexp>`, `PATROL_BS_COVERAGE_FORCE_COMPILE=true`), patrol gathers Dart line coverage from the running VM service and the Android test runner merges it into a JaCoCo `.exec` that BrowserStack collects. Requires `enableAndroidTestCoverage true` on the app's debug build type. Works with the Android test orchestrator on — the merged coverage file is always written to app-internal storage, never the shared/`coverageFile` path BrowserStack passes, since `clearPackageData` revokes the app's permission to write there between test runs — or off, which the app service supports by serving every test from one process; a missing JaCoCo runtime now warns loudly instead of silently skipping. (#3066)
