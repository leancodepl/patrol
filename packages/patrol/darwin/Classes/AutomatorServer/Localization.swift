...
class Localization {
    ...
    static func getLocalizedString(key: String) -> String {
        ...
        do {
            ...
            if let path = bundle.path(forResource: "Localizable", ofType: "strings", inDirectory: "\(targetLanguage).lproj") {
                return try getLocalizedStringForLanguage(key: key, language: targetLanguage, bundle: bundle, path: path)
            } else {
                Logger.shared.i("Could not find Localizable.strings for \(targetLanguage), trying English strings as fallback")
                ...
                if targetLanguage != "en" {
                    ...
                    Logger.shared.e("Localizable.strings file not found for language: \(targetLanguage) and no English fallback.")
                    return key // Fallback: return the key itself
                }
            }
        } catch let error as LocalizationError {
            Logger.shared.e("Localization error for key '\(key)' in language '\(targetLanguage)': \(error.localizedDescription)")
            return key // Fallback: return the key itself on error
        } catch {
            Logger.shared.e("An unexpected error occurred during localization for key '\(key)': \(error.localizedDescription)")
    ...
    private static func getLocalizedStringForLanguage(key: String, language: String, bundle: Bundle, path: String? = nil) throws -> String {
        let finalPath = path ?? bundle.path(forResource: "Localizable", ofType: "strings", inDirectory: "\(language).lproj")
            
        guard let filePath = finalPath else {
            throw LocalizationError.resourceNotFound("Localizable.strings file not found for language: \(language)")
        }
        ...
        guard let dictionary = NSDictionary(contentsOfFile: filePath) as? [String: String] else {
            ...
            throw LocalizationError.parsingFailed("Could not parse Localizable.strings for language: \(language)")
        }
            
        guard let localizedString = dictionary[key] else {
            throw LocalizationError.keyNotFound("Key '\(key)' not found in Localizable.strings for language: \(language)")
        }
