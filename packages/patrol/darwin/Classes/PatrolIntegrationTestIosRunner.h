...
// Common interface and basic methods shared between both implementations
#define PATROL_COMMON_INTERFACE(__test_class)                      \
...
// Common test setup and dart test discovery logic
#define PATROL_COMMON_TEST_SETUP                                                                              \
...
#define PATROL_COMMON_TEST_EXECUTION(__dart_test_name, __skip)                                            \
  if (__skip) {                                                                                           \
    XCTSkip(@"Skip that test \"%@\"", __dart_test_name);                                                  \
  }                                                                                                       \
                                                                                                          \
  __block ObjCRunDartTestResponse *response = NULL;                                                       \
  __block NSError *error;                                                                                 \
  [appServiceClient                                                                                       \
      runDartTestWithName:__dart_test_name                                                                \
               completion:^(ObjCRunDartTestResponse *_Nullable r, NSError *_Nullable err) {               \
                 ...
                 NSLog(@"runDartTest(\"%@\"): call finished, test result: %@", __dart_test_name, status); \
               }];                                                                                        \
                                                                                                          \
...
// Common invocation creation logic
#define PATROL_CREATE_INVOCATION(__dart_test_name, __implementation)                                        \
  SEL selector = NSSelectorFromString(__dart_test_name);                                                    \
  class_addMethod(self, selector, __implementation, "v@:");                                                 \
                                                                                                            \
  NSMethodSignature *signature = [self instanceMethodSignatureForSelector:selector];                        \
  NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:signature];                        \
  invocation.selector = selector;                                                                           \
                                                                                                            \
  NSLog(@"RunnerUITests.testInvocations(): selectorName = %@, signature: %@", __dart_test_name, signature); \
                                                                                                            \
  [invocations addObject:invocation];

#ifdef CLEAR_PACKAGE_DATA
#define PATROL_INTEGRATION_TEST_IOS_RUNNER(__test_class)                                                        \
  PATROL_COMMON_INTERFACE(__test_class)                                                                         \
                                                                                                                \
  +(void)uninstallApp {                                                                                         \
  ...
  +(NSArray<NSInvocation *> *)testInvocations {                                                                 \
    ...
    for (NSUInteger i = 0; i < dartTests.count; i++) {                                                          \
      ...
      IMP implementation = imp_implementationWithBlock(^(id _self) {                                            \
        ...
        [[[XCUIApplication alloc] init] launch];                                                                \
        PATROL_COMMON_TEST_EXECUTION(dartTestName, skip);                                                       \
      });                                                                                                       \
      /* Step 2 – create invocations to the dynamically created methods */                                      \
      PATROL_CREATE_INVOCATION(dartTestName, implementation);                                                   \
    }                                                                                                           \
                                                                                                                \
    return invocations;                                                                                         \
  }                                                                                                             \
