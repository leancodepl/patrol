...
- Surface the Patrol DevTools extension URL in web develop mode, so `--open-devtools` works there too. Note that the extension's native UI tree is Android/iOS only and reports "Unsupported platform" on web.
