...
- Add `screenshot_on_failure` option to the pubspec's `patrol` section, forwarded to the app (via a dart-define) across `build`, `test`, and `develop` so patrol can capture native failure screenshots (Android/BrowserStack). Off by default. (#3222)
