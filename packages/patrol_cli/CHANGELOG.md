...
- `patrol test` (Android) now pulls native screenshots from the device into a local `screenshots/` directory after the run when `screenshot_on_failure` is enabled, so failure screenshots are available from local/emulator runs, not only device farms. (#3222)
