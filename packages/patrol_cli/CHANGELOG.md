...
- Fix `patrol develop` hanging on iOS simulators with `Hot Restart: not attached to the app yet`, by attaching to the Dart VM service URL read from the device logs instead of relying on `flutter attach` discovery. (#3263)
