...
class IOSTestBackend {
  ...
  Future<void> build(IOSAppOptions options) async {
    await _disposeScope.run((scope) async {
      ...
      await disableXcodeDiagnosticCollection(xctestRunFile);
  ...
  /// Disables Xcode's post-test diagnostics, which can block a completed test
  /// run for 10 minutes or prompt for a password on a physical device.
  @visibleForTesting
  Future<void> disableXcodeDiagnosticCollection(String xctestRunPath) async {
    const plistKey =
    ...
    if (result.exitCode == 0) {
      _logger.detail('Disabled Xcode diagnostic collection in xctestrun');
    } else {
      _logger.warn(
        'Failed to disable Xcode diagnostic collection in xctestrun: '
        '${result.stdErr}',
      );
    }
  }
