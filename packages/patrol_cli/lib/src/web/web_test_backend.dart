...
class WebTestBackend {
  ...
  /// Frees [_restartInFlight] if `flutter run` reports neither success nor
  /// failure, so one unrecognised outcome can't wedge every later restart.
  Timer? _restartWatchdog;

  /// How long to wait for a restart outcome before giving up on it. Generous,
  /// because a cold incremental compile of a large app is not fast.
  static const _restartTimeout = Duration(minutes: 3);

  /// Flutter output collected since the last restart request, so a failed
  /// restart can hand the compiler errors back instead of just "it failed".
  final _restartOutput = <String>[];

  /// Reports restart outcomes to the caller ([develop]'s owner).
  void Function(HotRestartResult result)? _onHotRestart;

  /// The test the next hot restart will run. Tracked separately from the
  /// options passed to [develop], which never change, so that the target can
  /// be swapped mid-session.
  String? _developTarget;
  ...
  Future<void> develop(
    ...
    _developTarget = options.flutter.target;
    ...
    final stdinSubscription = stdin.listen((event) async {
      ...
      if (char == 'r' || char == 'R') {
        await _requestHotRestart(_developTarget ?? options.flutter.target);
  ...
  Future<void> _requestHotRestart(String target) async {
    ...
    if (process == null) {
      _dropRestart('the Flutter process is gone');
      return;
    }
    if (!_hotRestartActive) {
      _logger.warn('Hot Restart: not attached to the app yet!');
      _dropRestart('the app is not attached yet');
      return;
    }
    if (_restartInFlight) {
      _logger.warn('Hot Restart: a restart is already in progress');
      _dropRestart('another restart is still in progress');
      return;
    }

    _restartInFlight = true;
    _restartOutput.clear();
    _restartWatchdog?.cancel();
    _restartWatchdog = Timer(_restartTimeout, () {
      if (!_restartInFlight) {
        return;
      }
      _finishRestart(
        HotRestartResult.failed(
          'Flutter reported no outcome for the hot restart within '
          '${_restartTimeout.inMinutes} minutes.',
        ),
      );
    });
  ...
  /// The restart never reached `flutter run`. Callers are waiting on an
  /// outcome that will never arrive, so say so instead of only logging.
  void _dropRestart(String reason) {
    _onHotRestart?.call(
      HotRestartResult.dropped('Hot restart was not performed: $reason.'),
    );
  }

  void _finishRestart(HotRestartResult result) {
    _restartInFlight = false;
    _restartWatchdog?.cancel();
    _restartWatchdog = null;
    _restartOutput.clear();
    _onHotRestart?.call(result);
  }

  /// Swaps the test the next hot restart will run. The bundle itself is
  /// regenerated by the caller; this only keeps the log line honest.
  void setDevelopTarget(String target) => _developTarget = target;

  static const _maxRestartOutputLines = 60;

  void _collectRestartOutput(String line) {
    if (line.trim().isEmpty) {
      return;
    }
    _restartOutput.add(line);
    if (_restartOutput.length > _maxRestartOutputLines) {
      _restartOutput.removeAt(0);
    }
  }
  ...
  Future<void> _watchFlutterRun(Process process, Completer<void> exited) async {
    ...
    process.stderr
        ...
        .transform(const LineSplitter())
        .listen((line) {
          // Compile errors land here, and they are the whole reason a caller
          // wants to know why a hot restart failed.
          if (_restartInFlight) {
            _collectRestartOutput(line.replaceFirst(_flutterLogPrefix, ''));
          }
    ...
    process.exitCode.then((exitCode) {
      ...
      if (_restartInFlight) {
        _finishRestart(
          HotRestartResult.dropped(
            'Flutter exited (code $exitCode) before the hot restart completed.',
          ),
        );
      }
      _restartWatchdog?.cancel();
  ...
  void _onFlutterLine(String rawLine) {
    ...
    if (_restartInFlight) {
      _collectRestartOutput(line);
    }

    if (line.startsWith('Restarted application in')) {
      _logger.success(line);
      _finishRestart(const HotRestartResult.succeeded());
      return;
    }
    ...
    if (line == 'Try again after fixing the above error(s).' ||
      ...
      _finishRestart(
        HotRestartResult.failed(
          _restartOutput.isEmpty
              ? 'Hot restart failed to recompile the application.'
              : 'Hot restart failed to recompile the application:\n'
                    '${_restartOutput.join('\n')}',
        ),
      );
