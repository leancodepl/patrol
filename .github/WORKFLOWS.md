...
| [test locales on android device][test-android-locales] | Every 12h | Pixel7 | 36 | Flutter 3.38.x (stable) | `locale_testing_android` | Tests locale support on emulator.wtf for `en_US`, `fr_FR`, `de_DE`, and `pl_PL`. Uses `--device '[model=...,version=...,locale=...]'`. Excludes `web/`, `native_tests/`, and `volume_test.dart`. |
