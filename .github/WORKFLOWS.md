...
Package-level `pana` scoring only runs for packages published to pub.dev (`patrol`, `patrol_cli`, `patrol_finders`, `patrol_log`, `adb`, `patrol_mcp`) — `patrol_devtools_extension` and `patrol_gen` set `publish_to: none` and are skipped. Each of those prepare workflows ends its main job with a "Score with pana" step running the [pana-score][pana-score-action] composite action from `leancodepl/mobile-tools`, which scores the package the same way pub.dev would, publishes a `pana (<package>)` commit status, and writes a Markdown breakdown to the job summary. On matrix jobs (`patrol_cli`, `patrol_mcp`) the step is guarded to run on a single leg so the score is only reported once. The step uses `continue-on-error: true` so a low score is informational and never blocks the PR.
