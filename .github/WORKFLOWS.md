...
| [test patrol develop][test-patrol-develop] | PR (opened/synchronize on package, e2e_app, and schema changes; excludes docs), manual | Flutter 3.38.x (stable) | — | Tests `patrol develop` command on Linux (Android emulator, API 34) and macOS (iOS simulator: iPhone 16 Pro on iOS 26.2). The macOS job pins simulator runtime and passes `--ios 26.2` to `patrol_develop_test.dart` to keep xcode destination selection deterministic. Timeout: 30 minutes per job. |
...
Package-level `pana` scoring only runs for packages published to pub.dev (`patrol`, `patrol_cli`, `patrol_finders`, `patrol_log`, `adb`, `patrol_mcp`) — `patrol_devtools_extension` and `patrol_gen` set `publish_to: none` and are skipped. Each `pana-score` job runs the [pana-score][pana-score-action] composite action from `leancodepl/mobile-tools`, which scores the package the same way pub.dev would, publishes a `pana (<package>)` commit status, and writes a Markdown breakdown to the job summary. The step uses `continue-on-error: true` so a low score is informational and never blocks the PR.
