...
| test ios simulator | `test-ios-simulator.yaml` | Schedule (monthly on 1st), manual | Flutter 3.32.x (stable) | `ios && simulator` | Runs E2E tests on iOS simulator (iPhone 16) on iOS 26.0. Excludes `web/` directory, `volume_test.dart`, and `service_bluetooth_test.dart`. Records video and logs. Uses xcresultparser to generate JUnit reports and converts them to CTRF format for test reporting. Timeout: 100 minutes. Runs with `--full-isolation` flag. |
